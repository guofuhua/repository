...
QList<CSysMemInfoElement> CGetProcessInfo::getSysMemInfoList()
{
    ...
    for(i = 0; i < m_pid_group.size(); i++)
    {
        ...
        sys_process_mem.m_occupancyRate = 100 * (sys_process_mem.m_VmRSS * 1.0) / (m_totalmem * 1.0);
        m_sysMeminfo.append(sys_process_mem);
    }
    return m_sysMeminfo;
}

QList<CSysMemInfoElement> CGetProcessInfo::getSysMemInfoList(QList<int> processPid)
{
    QList<CSysMemInfoElement> m_sysMeminfo;
    int i = 0;
    m_sysMeminfo.clear();
    TRACE("processPid.size:%d\n",processPid.size());
    for(i = 0; i < processPid.size(); i++)
    {
        TRACE("processPid.size(%d):%d\n", i, processPid.at(i));
        CSysMemInfoElement sys_process_mem = getProcessMeminfo(processPid.at(i));
        sys_process_mem.m_occupancyRate = 100 * (sys_process_mem.m_VmRSS * 1.0) / (m_totalmem * 1.0);
        m_sysMeminfo.append(sys_process_mem);
    }
    return m_sysMeminfo;
